...
def spspk_overlay(fibers=None, ctr=[0., 0.], angle=0., which='science',
                  **kwargs):
    '''
    ...
    field_height = np.max(ctr[1] + fibers['dec']) - \
        np.min(ctr[1] - fibers['dec'])
...
def kde_errors(x, bandwidth, objname, e=None, cv=3, offset=0.):
    ...
    xw = np.ptp(xoffset)
    x_grid = np.linspace(
        xoffset.min() - 0.25*xw, xoffset.max() + 0.25*xw, 10000)
    ...
    # now sample the pdf to estimate the stdev of the distribution about the
    # maximum likelihood
...
def rejection_sample_2d(x, y, z, nper=100):
    ...
    # print np.repeat(coords.reshape(len(number), 2),
    # number.flatten()[:,np.newaxis])
